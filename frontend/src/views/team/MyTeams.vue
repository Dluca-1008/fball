  ...
  <div class="page-shell">
    <div class="page-header">
      <div class="header-left">
        <div class="header-icon teams">🏠</div>
        <div>
          <h1 class="header-title">我的球队</h1>
          <p class="header-desc">管理我所属的所有球队</p>
        </div>
      </div>
    ...
    <div class="team-cards" v-loading="loading">
      <el-empty v-if="!loading && teams.length === 0" description="您还没有加入任何球队，去创建一支吧" />
      <div v-for="team in teams" :key="team.id" class="team-card" @click="$router.push(`/app/teams/${team.id}`)">
        <div class="team-card-header">
          ...
          <el-button type="primary" link size="small" @click.stop="$router.push(`/app/teams/${team.id}`)">查看详情 →</el-button>
        </div>
...
.header-icon {
  ...
  font-size: 24px; box-shadow: 0 4px 12px rgba(103, 194, 58, 0.3);
}
...
.btn-create:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(64, 158, 255, 0.4); }
